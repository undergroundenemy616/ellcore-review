...
"""
Лучше использование UUID (universally unique identifier) вместо целочисленных значений (int) в качестве первичных ключей в базе т.к:

1. Глобальная уникальность: UUID гарантированно уникален не только внутри таблицы, но и во всем мире. 
Это особенно полезно для распределенных систем и синхронизации данных между различными базами данных.

2. Масштабируемость: В распределенных системах или при использовании техник шардирования (разделения данных по нескольким серверам) 
автоинкрементные ID могут вызвать проблемы с синхронизацией и уникальностью. UUID решает эти проблемы, 
поскольку генерируется уникально в любой точке.
"""

"""
Также лучше добавить поля date_created и date_updated. Они предоставляют важную информацию об истории записи и 
могут быть полезны по многим причинам:

1. Аудит и отслеживание: Ваши данные могут быть подвержены аудиту в целях безопасности, отчетности или 
соответствия нормативным требованиям. Зная, когда была создана или изменена запись, вы можете отслеживать и 
анализировать историю данных и понять, как они менялись со временем.

2. Отладка и тестирование: При возникновении проблем с данными эти поля могут помочь вам понять, когда и как 
произошли изменения, что может упростить процесс отладки и тестирования.
"""
...
# 1. Мне не очень понятно лошика создания суперпользователя, она неочевидна, почему тут магическое значение 2?
# 2. Подобные функциональности лучше выносить в отдельные функции
conn = engine.connect()
...
# создание тестовых данных (все что ниже - удалить) | Раз это нужно удалить, то почему это не удалено?
