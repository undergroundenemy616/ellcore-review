...
function startGetStocksWB() {
  ...
  sheetSettings.getRange('B4').setValue(false); 
  console.info('startGetStocksWB', `Активация формул отключена`);


  let sheet = ss.getSheetByName('Выгрузка');
  let mr = sheet.getMaxRows();
  if (mr > 1) sheet.getRange(`2:${mr}`).clearContent(); // Очистим строки от 2 строки до конца листа
  
  console.info('startGetStocksWB', `Старые значения на листе "Выгрузка" удалены`);

  setTrigger('launchGetStocksWB');
};
...
function launchGetStocksWB() {
  ...
  for (let i = 0; i < dataKEY.length; i++) {
    ...
    console.info('launchGetStocksWB', 'Все функции успешно выполнены');
    console.timeEnd('Время выполнения');

    return;
  };

  deleteTriggers('launchGetStocksWB');

  lock.releaseLock(); // Снимаем блокировку

  sheetSettings.getRange('B4').setValue(true); 
  console.info('launchGetStocksWB', `Активация формул включена`);
